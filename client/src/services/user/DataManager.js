...
class DataManager {
    static API_BASE_URL = process.env.REACT_APP_PROD || 'http://localhost:3001/api';
    ...
    async checkAuth(setLoading) {
        try {
            const response = await fetch(`${DataManager.API_BASE_URL}/auth/current_user`, {
                credentials: 'include'
            });
    ...
    async updateUserData(userId, userData) {
        try {
            ...
            const response = await fetch(`${DataManager.API_BASE_URL}/users/${userId}`, {
                method: 'PUT',
                headers: {
            ...
            this.handleError(error, 'Error updating user data:', this.setters.setError);
        }
    }
