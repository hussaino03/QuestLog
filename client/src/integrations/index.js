...
const IntegrationsDropdown = ({ addTask, isAuthenticated }) => {
  ...
  const handleTickTickImport = () => {
    importFromTickTick(addTask, setIsLoading);
    setIsOpen(false);
  };
            ...
            <div className="absolute right-0 mt-2 py-1 w-auto bg-white dark:bg-gray-800 border-2 
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    <svg className="w-8 h-8" viewBox="0 0 24 24" fill="#2196F3">
                      <path d="M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm-1.959 17l-4.5-4.319 1.395-1.435 3.08 2.937 7.021-7.183 1.422 1.409-8.418 8.591z"/>
                    </svg>
                    <span className="text-sm font-medium">Import from TickTick</span>
                  </>
                )}
              </button>
            </div>
