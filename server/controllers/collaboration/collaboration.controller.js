...
async function shareProject(req, res) {
  try {
    ...
    console.log(
      `[SHARE] Attempting to share project ${taskId} with user ${userId}`
    );
...
async function updateSharedProject(req, res) {
  try {
    ...
    console.log(
      `[UPDATE] Updating task ${taskId}, subtask ${subtaskIndex} to ${completed}`
    );
    ...
    const usersWithTask = await usersCollection
      .find({
        $or: [{ 'tasks.id': taskId }, { 'sharedProjects.projectId': taskId }]
      })
      .toArray();
...
async function updateSharedProjectDetails(req, res) {
  try {
    ...
    console.log(
      `[UPDATE-DETAILS] Updating project ${taskId} details:`,
      updatedDetails
    );
    ...
    const usersWithTask = await usersCollection
      ...
      .toArray();
    console.log(
      `[UPDATE-DETAILS] Found ${usersWithTask.length} users with this project`
    );

    for (const user of usersWithTask) {
      console.log(
        `[UPDATE-DETAILS] Updating project for user ${user.name} (${user._id})`
      );
